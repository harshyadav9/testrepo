...
export default function StudentInSlot() {
              ...
              <div className="shadow bg-white rounded-16">
                <div className="p-4 " >
                  ...
                  <div className="row">
                    ...
                    {
                      slots.filter(s => s.examTheme === "GO4U").length !== 0 &&
                      (
                        <div className="col-sm">
                          <div className="form-wrapper">
                            ...
                            <select onChange={e => {
                              chooseSlot('GO4U', e.target.value);
                              setExamTypes(ev => ({
                                ...ev,
                                ['GO4U']: true,
                              }))
                              ...
                              <option value="volvo" >Select Slot</option>
                              {
                                slots && Array.isArray(slots) ? slots.filter(s => s.examTheme === "GO4U").map(slot => (
                                  // <option value={slot.slotId}>{dayjs(slot?.dateOfExam).format('DD-MM-YYYY')}/{slot?.slotDatetime}</option>
                                  <option value={slot.slotID}>{dayjs(slot?.dateofExam).format('DD-MM-YYYY')}/{slot?.slotdatetime}</option>
                                )) : null
                              }
                            ...
                            <a href="javascript:void(0)" className="check-slot d-inline-block mt-2 font-bold" data-toggle="modal" data-target="#myModalexam"
                              onClick={_ => togglePop(slots.filter(s => s.examTheme === "GO4U"))}
                    ...
                    {
                      slots.filter(s => s.examTheme === "MOCKGO4U").length !== 0 &&
                      (
                        <div className="col-sm">
                          <div className="form-wrapper">
                            <div className="form-wrapper">
                              ...
                              <select class="dropdown-school" id="cars" onChange={e => {
                                ...
                                {
                                  slots && Array.isArray(slots) ? slots.filter(s => s.examTheme === "MOCKGO4U").map(slot => (
                                    <option value={slot.slotID}>{dayjs(slot?.dateofExam).format('DD-MM-YYYY')} / {slot?.slotdatetime}</option>
                                  )) : null
                                }
                              </select>
                              ...
                              <a href="javascript:void(0)" className="check-slot d-inline-block mt-2 font-bold" data-toggle="modal" data-target="#myModalexam" onClick={_ => togglePop(slots.filter(s => s.examTheme === "MOCKGO4U"))}>
                                <svg className="icon align-middle">
                                  <use xlinkHref="#check-slot"></use>
